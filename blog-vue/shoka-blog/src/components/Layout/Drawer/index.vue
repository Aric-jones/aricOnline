	...
	>
		<n-drawer-content style="padding-top: 0.5rem">
			<div class="author-container">
				<img
					class="author-avatar"
					:src="blog.blogInfoSafe.siteConfig.authorAvatar"
				/>
				<p class="author-name">{{ blog.blogInfoSafe.siteConfig.siteAuthor }}</p>
				<div class="site-desc">
					{{ blog.blogInfoSafe.siteConfig.siteIntro }}
				</div>
			</div>
			<blog-info></blog-info>
			...
			<ul class="side-menu">
				<template v-for="menu of menuList" :key="menu.name">
					...
					<li
						v-else
						class="item dropdown"
						:class="{ expand: expand(menu.children) }"
					>
						<a
							><svg-icon :icon-class="menu.icon"></svg-icon> {{ menu.name }}
						</a>
